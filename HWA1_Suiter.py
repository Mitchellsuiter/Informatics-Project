...
marriagerates.to_sql(con=conn,                                          #connect the database to sql, name it and specify what to do if it already exists
                name='MarriageRates',
                if_exists='replace')
cur = conn.cursor()                                                     # create a cursor so we can run an sql command in the next line
cur.execute("SELECT * FROM MarriageRates;")                             #select all of the info from our database marriagerates
database = cur.fetchall()                                                #save the results as a variable
